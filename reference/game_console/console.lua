SpeakTypesSettings = {
  none = {},
  say = { speakType = MessageModes.Say, color = '#cab58c' }, -- all #cab58c were #FFFF00.
  whisper = { speakType = MessageModes.Whisper, color = '#cab58c' },
  yell = { speakType = MessageModes.Yell, color = '#ffb23a' },
  ...
  privatePlayerToNpc = { speakType = MessageModes.NpcTo, color = '#3d7f8e', private = true, npcChat = true },
  privateNpcToPlayer = { speakType = MessageModes.NpcFrom, color = '#a8e051', private = true, npcChat = true },
  channelYellow = { speakType = MessageModes.Channel, color = '#cab58c' },
  ...
  monsterSay = { speakType = MessageModes.MonsterSay, color = '#f3922a', hideInConsole = true},
  monsterYell = { speakType = MessageModes.MonsterYell, color = '#f3922a', hideInConsole = true},
  rvrAnswerFrom = { speakType = MessageModes.RVRAnswer, color = '#FE6500' },
  rvrAnswerTo = { speakType = MessageModes.RVRAnswer, color = '#FE6500' },
  rvrContinue = { speakType = MessageModes.RVRContinue, color = '#cab58c' },
}
...
local keybindChatSendMessage = KeybindManager:getKeybindByName("Chat Send Message", "Interface")

local keybindChatHistoryPrev = KeybindManager:getKeybindByName("Chat History Previous", "Interface")

local keybindChatHistoryNext = KeybindManager:getKeybindByName("Chat History Next", "Interface")

local keybindChatTabPrev = KeybindManager:getKeybindByName("Chat Tab Previous", "Interface")

local keybindChatTabNext = KeybindManager:getKeybindByName("Chat Tab Next", "Interface")

local keybindChatCleanAll = KeybindManager:getKeybindByName("Chat Clean All", "Interface")

local keybindChatModeSwitch = KeybindManager:getKeybindByName("Chat Mode Switch", "Interface")

local keybindChatChannelManager = KeybindManager:getKeybindByName("Chat Channel Manager", "Interface")
local keybindChatTabClose = KeybindManager:getKeybindByName("Chat Tab Close", "Interface")
local keybindShowHelp = KeybindManager:getKeybindByName("Show Help", "Modules")

local keybindMinimapMoveLeft = KeybindManager:getKeybindByName("Minimap Move Left", "Interface")
local keybindMinimapMoveRight = KeybindManager:getKeybindByName("Minimap Move Right", "Interface")
local keybindMinimapMoveUp = KeybindManager:getKeybindByName("Minimap Move Up", "Interface")
local keybindMinimapMoveDown = KeybindManager:getKeybindByName("Minimap Move Down", "Interface")
local keybindMinimapFullscreen = KeybindManager:getKeybindByName("Minimap Fullscreen", "Interface")

--------

local notificationChannel = g_sounds.getChannel(1)
local wasTopMenuVisible = nil
...
function enableChat()
  ...
  consoleTextEdit:setColor("#3d7f8e")
  consoleTextEdit:setCursorVisible(true)
  consoleTextEdit:focus()

  if KeybindManager:getChatMode() == CHAT_MODE_MODERN then
    KeybindManager.keybindsMuted = true
  else
    KeybindManager.keybindsMuted = false
  end
end

function disableChat()
  local gameInterface = modules.game_interface
  consoleTextEdit:setEditable(false)
  consoleTextEdit:setText("Press [" .. (KeybindManager:getChatMode() == CHAT_MODE_MODERN and keybindChatSendMessage.altKeyCombo or keybindChatSendMessage.keyCombo) .. "] to start typing...")
  consoleTextEdit:setColor("#1E5968")
  consoleTextEdit:setCursorVisible(false)
  KeybindManager.keybindsMuted = false
  gameInterface:focus()
end

function pressChatSendButton()
  if KeybindManager:getChatMode() == CHAT_MODE_MODERN then
  ...
  if KeybindManager:getChatMode() == CHAT_MODE_MODERN then
    disableChat()
    modules.game_playercontrols.stopSmartWalk() -- Purge smartwalk cache since we could have interrupted it by typing while key(s) were held and corrupted it.
  end
end

function MinimapOnGeometryChange(self)
  local bottomSplitter = rootWidget:getChildById("gameRootPanel"):getChildById("bottomSplitter")
  if self.originalMarginTop == nil then
    self.originalMarginTop = self:getMarginTop()
  end
  local add = bottomSplitter:getMarginBottom() - self:getHeight() - self.originalMarginTop
  if add < 0 then
    self:setMarginTop(self.originalMarginTop + add)
  else
    self:setMarginTop(self.originalMarginTop)
  end
...
function init()
  ...
  -- Implement Ctrl+C functionality.
  ...
  keybindChatHistoryPrev:setWidgetToBindTo(consolePanel)
  keybindChatHistoryNext:setWidgetToBindTo(consolePanel)
  keybindChatTabPrev:setWidgetToBindTo(consolePanel)
  keybindChatTabNext:setWidgetToBindTo(consolePanel)
  keybindChatCleanAll:setWidgetToBindTo(consolePanel)
  keybindChatModeSwitch:setWidgetToBindTo(consolePanel)
  keybindChatSendMessage:setWidgetToBindTo(consolePanel)

  keybindChatHistoryPrev:activate()
  keybindChatHistoryNext:activate()
  keybindChatTabPrev:activate()
  keybindChatTabNext:activate()
  keybindChatCleanAll:activate()
  keybindChatModeSwitch:activate()
  keybindChatSendMessage:activate()
  keybindChatChannelManager:activate()
  keybindChatTabClose:activate()
  keybindShowHelp:activate()
  ...
  if KeybindManager:getKeyComboMode() == CHAT_MODE_MODERN then
    disableChat()
  end

  -------- Minimap
  minimapWindow = consolePanel;

  minimapWidget = consolePanel:recursiveGetChildById('minimap')
  minimapParent = minimapWidget:getParent()
  local gameRootPanel = modules.game_interface.getRootPanel()

  keybindMinimapMoveLeft:activate()
  keybindMinimapMoveRight:activate()
  keybindMinimapMoveUp:activate()
  keybindMinimapMoveDown:activate()
  keybindMinimapFullscreen:activate()
  -------- End Minimap

  connect(g_game, {
    onGameStart = minimap_online,
    onGameEnd = minimap_offline,
  })
...
function isInChat()
  return consoleTextEdit:isFocused() and consoleTextEdit:isEditable()
end
...
function terminate()
  ...
  keybindChatChannelManager:deactivate()
  keybindChatTabClose:deactivate()
  keybindShowHelp:deactivate()
  ...
  if violationWindow then
    keybindPlayerReporter:deactivate()
  ...
  local gameRootPanel = modules.game_interface.getRootPanel()
  keybindMinimapMoveLeft:deactivate()
  keybindMinimapMoveRight:deactivate()
  keybindMinimapMoveUp:deactivate()
  keybindMinimapMoveDown:deactivate()
  keybindMinimapFullscreen:deactivate()
end
...
function toggleNotifications(consoleTabBar)
  if consoleTabBar.notifications == nil then
    consoleTabBar.notifications = false
  end
  consoleTabBar.notifications = not(consoleTabBar.notifications)
end

function toggleFullMap()
  if not fullmapView then
    fullmapView = true
    minimapWidget.fullscreen = true
    g_minimap.setElipsify(false)
    minimapWindow:hide()
    minimapWidget:setParent(modules.game_interface.getRootPanel())
    minimapWidget:fill('parent')
    minimapWidget:setAlternativeWidgetsVisible(true)
    minimapWidget:setBackgroundColor('#000000')
    local controlsPanel = minimapWidget:recursiveGetChildById('controlsPanelWrapper')
    controlsPanel:setVisible(true)
    local pos = { x = controlsPanel:getParent():getWidth()-controlsPanel:getWidth(), y = 0}
    controlsPanel:setPosition(pos)
    wasTopMenuVisible = modules.client_topmenu.isTopMenuVisible()
    if wasTopMenuVisible then
      local topMenu = modules.client_topmenu.hideGameButtons()
    end
    minimapWidget:focus()
  else
    fullmapView = false
    minimapWidget.fullscreen = false
    g_minimap.setElipsify(true)
    minimapWindow:show()
    minimapWidget:setParent(minimapParent)
    minimapWidget:fill('parent')
    minimapWidget:setAlternativeWidgetsVisible(false)
    minimapWidget:lower()
    minimapWidget:setBackgroundColor('alpha')
    local controlsPanel = minimapWidget:recursiveGetChildById('controlsPanelWrapper'):setVisible(false)
    if wasTopMenuVisible then
      local topMenu = modules.client_topmenu.showGameButtons()
    end
  end
...
function detachMinimap()
	local newMapHolder = g_ui.createWidget('MainWindow', rootWidget)
	newMapHolder:setSize("199 199")
	newMapHolder:addChild(minimapWidget)
...
function openPlayerReportRuleViolationWindow()
  ...
  local keybindPlayerReporter = KeybindManager:getKeybindByName("Show Player Reporter", "Modules")
  keybindPlayerReporter:setWidgetToBindTo(violationWindow)
  keybindPlayerReporter:activate()
...
function addPrivateChannel(receiver, focus)
  channels[receiver] = receiver
  local tab =  addTab(receiver, false)
  tab.notifications = true
  if focus then
  consoleTabBar:selectTab(tab)
  end
  return tab
...
function addPrivateText(text, speaktype, name, isPrivateCommand, creatureName)
  ...
  elseif focus then
   ...
   if privateTab.notifications  and not (g_window.isVisible() and g_window.isFocused())then
    notificationChannel:stop()
    notificationChannel:play("/sounds/notify",0,g_settings.get("musicSoundVolume")/100)
  end
...
function addText(text, speaktype, tabName, creatureName)
  ...
  if tab ~= nil then
    ...
    if tab.notifications  and not (g_window.isVisible()  and g_window.isFocused())then
      notificationChannel:stop()
      notificationChannel:play("/sounds/notify",0,g_settings.get("musicSoundVolume")/100)
    end
  end
...
function createHyperlink(text, callback)
	consoleTextEdit:setText(consoleTextEdit:getText() .. "{ \215 : " .. text .. "}")
end

function beautifyHyperlink(headertype, data)
  -- If we send a header 'h', we want the game_help to open and show an article.
  -- Data is: {nameOfArticle, articleID}
  if headertype == "%{%@h%:" then
      return "\221 [Help Article: " .. data[1] .. " < " .. data[2] .. "] "
  end
  -- If we send a header 'http or https', we want to open an actual hyperlink in a browser.
  -- Data is: {nameOfArticle, articleID}
  if headertype == "%{http%:" or headertype == "%{https%:" then
      if headertype == "%{http%:" then headertype = "http:" end
      if headertype == "%{https%:" then headertype = "http:" end
      return "\204 [External Link: " .. headertype .. "" .. data .. "] "
  end

  -- If we send a header 'c', we want the game_outfitwindow to try and save a new outfit preset.
  -- Data is: {presetName, head, primary, secondary, detail}
  if headertype == "%{%@c%:" then
      return "\208 [Outfit Color Preset: " .. data[1] .. " (" .. data[2] .. ";" .. data[3] .. ";" .. data[4].. ";" .. data[5] .. ") - Click to try on!]"
  end

  return nil
end

function deconstructHyperlink(hyperlink)
   -- Deconstructs a hyperlink (a beautified one) and dissects it for data we need to gather from it in order to do something when it is clicked.

   print("Attempting to deconstruct: " .. hyperlink .. "")

   local results = {
      type = "", startpos = 0, endpos = 0, capture = "", data = {}
   }

   -- Check if this is a Help Article
    if string.find(hyperlink, "%[Help Article%:") ~= nil then
        results.startpos, results.endpos, results.capture = string.find(hyperlink, "%<(.-)%]")
        results.data = split(results.capture, ";")
        results.type = "Help Article"
        print("Captured help article from " .. results.startpos .. " to " .. results.endpos.. ". Capture: " .. results.capture .. "")

   -- Check if this is a Website Link
    elseif string.find(hyperlink, "%[External Link%:") ~= nil then
        results.startpos, results.endpos, results.capture = string.find(hyperlink, "%[External Link%:%s(.-)%]")
        results.data = results.capture
        results.type = "Website Hyperlink"

    -- Check if this is an Outfit Preset Link
    elseif string.find(hyperlink, "%[Outfit Color Preset%:") ~= nil then
             results.startpos, results.endpos, results.capture = string.find(hyperlink, "%[Outfit Color Preset%:%s(.-)%]")
             results.startpos, results.endpos, results.capture = string.find(results.capture, "%((.-)%)")
             results.data = split(results.capture, ";")
             results.type = "Outfit Color Preset"
             table.insert(results.data, string.trim(string.match(hyperlink, "%[Outfit Color Preset%:%s(.-)%(")))
    -- End type checks.
    end


    -- Do something after we have all the data and we have determined the type of link that was clicked:

    if results.type == "Help Article" then
		  local id = tonumber(results.data[1])
		  modules.game_help.fillTutorialDisplay(id)
		  modules.game_help.show()
    end

    if results.type == "Website Hyperlink" then
		  local yesCallback = function()
			if confirmWebsiteVisitWindow then
				g_platform.openUrl(results.data)
				confirmWebsiteVisitWindow:destroy()
				confirmWebsiteVisitWindow=nil
			end
		  end
		...
		confirmWebsiteVisitWindow = displayGeneralBox(tr('Warning!'), tr("The link you just clicked leads to an external source on the internet. It will open in your internet browser.\nAre you absolutely sure you want to visit this link?\n\n\211 WARNING \211 - Links can lead to dangerous, malicious, and inappropriate websites.\nNecroniaTeam holds no responsibility over the content you visit outside of the game)"), {
      { text=tr('Visit Link'), callback=yesCallback },
      { text=tr('Close'), callback=noCallback },
      anchor=AnchorHorizontalCenter}, yesCallback, noCallback)
    end

    if results.type == "Outfit Color Preset" then
        print("Outfit color preset link clicked - data")
        modules.game_outfitwindow.createPreviewWindow(results.data[5], tonumber(results.data[1]), tonumber(results.data[2]), tonumber(results.data[3]), tonumber(results.data[4]))
    end

   return true
end

function filterForHyperlink(msg)
    -- Checks a given 'msg' string for occurrences of hyperlinks and deconstructs the details of those links to produce actual meaning later.
    -- Hyperlinks looks like "{@x:p1;p2;p3...}" where 'x' is a header letter. Each header represents which type of hyperlink it is.
    -- For example, a 'h' header would mean the parameters provided have something to do with opening a part of the game_help module.

    -- Config
    local headers = {"%{%@h%:","%{%@i%:","%{https%:", "%{http%:","%{%@c%:"}
    ...
    -- Scan for where the link is located within our message, and grab details from it. Process only one link, break after that.
    -- We can't have multiple links due to the way this hyperlink layer works in OTC. And hell no am I rewriting that.
    for _,header in pairs (headers) do
        if string.find(msg, header) ~= nil then
            chosenHeader = header
            results.startpos, results.endpos, results.capture = string.find(msg, header .. "(.-)%}")
            break
        end
    end

    -- If, after running the loop above, we still don't have a chosenHeader set - that means we never found a matching header. Abort this function, there is no hyperlink.
    if type(chosenHeader) ~= "string" then return nil end

    -- We have the result, now we have to deconstruct it and put each value in a table.
    local orderedData;
    if chosenHeader == headers[3] or chosenHeader == headers[4] then -- We are dealing with a real hyperlink, must do something different here.
        orderedData = results.capture
    else
        orderedData = split(results.capture, ";")
    end

    -- Beautify the result
    results.final = beautifyHyperlink(chosenHeader, orderedData)

    -- If beautified result came back nice from its function, return it, otherwise nil.
    if type(results.final) == "string" then
...
function addTabText(text, speaktype, tab, creatureName)
  --Player was losing focus over game map when farewell to npcs, This fixed it.
  scheduleEvent(function()
      if  g_game.isOnline() and tab == consoleTabBar:getTab('NPCs') then
      local gameRootPanel = rootWidget:getChildById("gameRootPanel")
      rootWidget:focusChild(gameRootPanel)
     end
  end,100)
  ...
  -- Recognize if sent text contains a link and reformulate the link to look better.

	local filterForLinkResults = filterForHyperlink(text)

	if type(filterForLinkResults) == "string" then
		text = creatureName .. " links to: {".. filterForLinkResults .. "}"
	   if modules.client_options.getOption('showTimestampsInConsole') then
			text = os.date('%H:%M') .. ' ' .. text
		end
	end
  ...
  -- Overlay for consoleBuffer which shows highlighted and linking words only
  if speaktype.npcChat and (g_game.getCharacterName() ~= creatureName or g_game.getCharacterName() == 'Account Manager') then
      ...
      local linkingData = getLinkingText(text)
      ...
      if #highlightData > 0 then
          ...
          labelHighlight:setColor("#25ffba")
	  ...
	  local linkingData = getLinkingText(text)
	  if #linkingData > 0 then
			local labelLink = g_ui.createWidget('ConsolePhantomLink', label)
			labelLink:fill('parent')
			labelLink:setId('consoleLabelLink' .. consoleBuffer:getChildCount())
			-- Remove the unnecessary link text
			for i = 1, #linkingData / 3 do
			  local dataBlock = { _start = linkingData[(i-1)*3+1], _end = linkingData[(i-1)*3+2], words = linkingData[(i-1)*3+3] }
			  text = text:gsub("%{(.-)%}", dataBlock.words, 1)

			  -- Recalculate positions as braces are removed
			  linkingData[(i-1)*3+1] = dataBlock._start - ((i-1) * 2)
			  linkingData[(i-1)*3+2] = dataBlock._end - (1 + (i-1) * 2)
			end
			label:setText(text)
		  -- Calculate the positions of the highlighted text and fill with string.char(127) [Width: 1]
		  local drawText = label:getDrawText()
		  ...
		  for i = 1, #linkingData / 3 do
			  local dataBlock = { _start = linkingData[(i-1)*3+1], _end = linkingData[(i-1)*3+2], words = linkingData[(i-1)*3+3] }
			  local lastBlockEnd = (linkingData[(i-2)*3+2] or 1)
			...
			-- Fill the highlight label to the same size as default label
			  local finalBlockEnd = (linkingData[(#linkingData/3-1)*3+2] or 1)
			  for letter = finalBlockEnd, drawText:len() do
				  local tmpChar = string.byte(drawText:sub(letter, letter))
				  local fillChar = (tmpChar == 10 or tmpChar == 32) and string.char(tmpChar) or string.char(127)
			  ...
			  labelLink:setText(tmpText)
			  labelLink.onClick = function(self)
					deconstructHyperlink(self:getText())
			  end
	  end
  end
...
function processChannelTabMenu(tab, mousePos, mouseButton)
  ...
  if consoleTabBar:getCurrentTab() == tab then
    ...
    menu:addOption(tr('Save Messages'), function()
      ...
      local filename = os.date('[%d-%m-%y %H-%M-%S] ') .. channelName .. '.txt'
      local filepath = '/' .. filename

      -- extra information at the beginning
      table.insert(lines, 1, os.date('Channel saved at %a %b %d %H:%M:%S %Y'))

      local lineEndings = '\n'
      --supposedly there is a better way to check OS but this will work as well
      if g_platform.getOSName():lower():match("windows") then
        lineEndings = '\r\n'
      end
      g_resources.writeFileContents(filepath, table.concat(lines, lineEndings))
      modules.game_textmessage.displayStatusMessage(tr('Channel appended to %s', filename))
    end)

    local toggle = (tab.notifications == nil or tab.notifications == false) and "Enable" or "Disable"
    menu:addOption(tr(toggle..' Sound Notifications '), function(self)
      toggleNotifications(tab)
      self:setText(tr(toggle..' Sound Notifications'))
    end)
  end
...
function processMessageMenu(mousePos, mouseButton, creatureName, text, label, tab)
  if mouseButton == MouseRightButton then
    ...
    local selection = tab.tabPanel:getChildById('consoleBuffer').selectionText
    if selection and #selection > 0 then
        menu:addOption(tr('Copy selected'), function() g_window.setClipboardText(selection) end, '(Ctrl+C)')
    end
    if text then
...
function sendMessage(message, tab)
  ...
  local findIni, findEnd, chatCommandInitial, chatCommandPrivate, chatCommandEnd,
    chatCommandMessage = message:find("([%*%@])(.+)([%*%@])(.*)")
...
function onOpenPrivateChannel(receiver)
  addPrivateChannel(receiver, true)
end
...
function processViolation(name, text)
  ...
  scheduleEvent(function() addTabText(text, SpeakTypesSettings.say, tab, name) end, 100)
end
...
function onClickIgnoreButton()
  ...
  ignoreListPanel.onChildFocused = function() removeIgnoreButton:enable() end
  ...
  whiteListPanel.onChildFocused = function() removeWhitelistButton:enable() end
...
function minimap_online()
  loadMap(not preloaded)
  updateCameraPosition()
end

function minimap_offline()
  saveMap()
end
...
function setMinimapLocation(value)
	if value == "right" then
		consolePanel:getChildById("minimapbg"):breakAnchors()
		consolePanel:getChildById("minimapbg"):addAnchor(AnchorBottom, 'parent', AnchorBottom)
		consolePanel:getChildById("minimapbg"):addAnchor(AnchorRight, 'parent', AnchorRight)
		consolePanel:getChildById("minimapbg"):setMarginLeft(0)
		consolePanel:getChildById("minimapbg"):setMarginRight(14)
	else
		consolePanel:getChildById("minimapbg"):breakAnchors()
		consolePanel:getChildById("minimapbg"):addAnchor(AnchorBottom, 'parent', AnchorBottom)
		consolePanel:getChildById("minimapbg"):addAnchor(AnchorLeft, 'parent', AnchorLeft)
		consolePanel:getChildById("minimapbg"):setMarginLeft(14)
		consolePanel:getChildById("minimapbg"):setMarginRight(0)
	end
end
